...
var dateHelper = (function () {
	...
	function formatTime(hour24, minute, format) {
		...
		switch (format) {
			...
			case 'H:i': return { value, text: `${pad(hour24)}:${pad(minute)}` };
			// 'h' = 12-hour with leading zeros
			case 'h:i A': return { value, text: `${pad(hour24 % 12 || 12)}:${pad(minute)} ${hour24 >= 12 ? 'PM' : 'AM'}` };
			case 'h:i a': return { value, text: `${pad(hour24 % 12 || 12)}:${pad(minute)} ${hour24 >= 12 ? 'pm' : 'am'}` };
			case 'h:i': return { value, text: `${pad(hour24 % 12 || 12)}:${pad(minute)}` };
			// 'g' = 12-hour without leading zeros
			case 'g:i A': return { value, text: `${hour24 % 12 || 12}:${pad(minute)} ${hour24 >= 12 ? 'PM' : 'AM'}` };
			case 'g:i a': return { value, text: `${hour24 % 12 || 12}:${pad(minute)} ${hour24 >= 12 ? 'pm' : 'am'}` };
			// 'G' = 24-hour without leading zeros
			case 'G.i': return { value, text: `${hour24}.${pad(minute)}` };
			case 'G:i': return { value, text: `${hour24}:${pad(minute)}` };
			default: return { value, text: value };
		}
	}

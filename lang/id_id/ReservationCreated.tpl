	...
	{if $Resources|default:array()|count > 1}
		<strong>Sumber Daya ({$Resources|default:array()|count}):</strong> <br />
	{else}
		<strong>Sumber Daya:</strong><br/>
	{/if}
	{foreach from=$Resources item=resource name=resourceLoop}
		<strong>{$resource.name|escape}</strong><br/>
		{if $resource.scheduleName}<strong>Jadwal:</strong> {$resource.scheduleName|escape}<br/>{/if}
		...
		{if $resource.attributeRows|default:array()|count > 0}
			<strong>Detail Sumber Daya:</strong><br/>
